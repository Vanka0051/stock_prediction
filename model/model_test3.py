        ...
        with tf.variable_scope("channel1"):
            with tf.variable_scope("CNN_part"):
                with tf.variable_scope("cnn_m20Ds"):
                    self.conv1_c1 = conv(self.inputX, 20, 1, 20, 1, 1, '1', name = 'integration', config = self.config) #conv1 :[batch_size, seq_length, feat_num, 1]
                ...
                with tf.variable_scope("cnn_3Ds"):
                    self.conv2_3_c1 = conv(self.relu1_c1, 3, self.config.feature_length, 1, self.config.feature_length, 20,'2_3', config = self.config)#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_5Ds"):
                    self.conv2_5_c1 = conv(self.relu1_c1, 5, self.config.feature_length, 1, self.config.feature_length, 20,'2_5', config = self.config)#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_7Ds"):
                    self.conv2_7_c1 = conv(self.relu1_c1, 7, self.config.feature_length, 1, self.config.feature_length, 20,'2_7', config = self.config)#conv2 :[batch_size, length, 1, channel]
                    self.bn2_7_c1 = bn(self.conv2_7_c1, self.is_training, '2_7')
                    self.relu2_7_c1 = tf.nn.relu(self.bn2_7_c1)#通过5×feat_len卷积提取特征，将所有特征按20种不同比例融合成20种新特征
                    self.relu2_7_c1 = tf.squeeze(self.relu2_7_c1, 2)                    
                with tf.variable_scope("cnn_9Ds"):
                    self.conv2_9_c1 = conv(self.relu1_c1, 9, self.config.feature_length, 1, self.config.feature_length, 20,'2_9', config = self.config)#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_maxpool"):
                    self.cnn_out_c1 = tf.concat([self.relu2_3_c1, self.relu2_5_c1, self.relu2_7_c1, self.relu2_9_c1], 2)#将新特征组合在一起
                    self.cnn_out_c1 = tf.expand_dims(self.cnn_out_c1, 3)
                    self.cnn1_c1 = conv(self.cnn_out_c1, 3, 3, 2, 2, 128, 'cnn1', config = self.config)#将组合特征再通过cnn和maxpool提取特征
                    self.cnn1_c1 = bn(self.cnn1_c1, self.is_training, '11')
                    self.maxpool1_c1 = max_pool(self.cnn1_c1)
                    self.cnn2_c1 = conv(self.maxpool1_c1, 3, 3, 2, 2, 256, 'cnn2', config = self.config)
                    self.cnn2_c1 = bn(self.cnn2_c1, self.is_training, '22')
                    self.maxpool2_c1 = max_pool(self.cnn2_c1)
                    self.cnn3_c1 = conv(self.maxpool2_c1, 3, 3, 2, 2, 512, 'cnn3', config = self.config)
                    self.cnn3_c1 = bn(self.cnn3_c1, self.is_training, '33')
                    self.maxpool3_c1 = max_pool(self.cnn3_c1, stride_h = 4, stride_w = 4)
            ...
            with tf.variable_scope("FC_part"):                  #全连接层将400维特征缩小为200维，再缩小为1。即：预测值
                self.fc1_c1 = fc(self.BiLSTM_out3_c1, 1024, num = '1')
                self.fc1_c1 = tf.nn.relu(self.fc1_c1)
                self.fc1_c1 = tf.nn.dropout(self.fc1_c1, keep_prob = self.dropout_rate)
#                self.fc2_c1 = fc(self.fc1_c1, 512, num = '2')


        with tf.variable_scope("channel2"):
            with tf.variable_scope("CNN_part"):
                with tf.variable_scope("cnn_m20Ds"):
                    self.conv1_c2 = conv(self.inputX, 20, 1, 20, 1, 1, '1', name = 'integration', config = self.config) #conv1 :[batch_size, seq_length, feat_num, 1]
                    self.bn1_c2 = bn(self.conv1_c2, self.is_training, '1')
                    self.relu1_c2 = tf.nn.relu(self.bn1_c2)
                with tf.variable_scope("cnn_3Ds"):
                    self.conv2_3_c2 = conv(self.relu1_c2, 3, self.config.feature_length, 1, self.config.feature_length, 20,'2_3', config = self.config)#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_5Ds"):
                    self.conv2_5_c2 = conv(self.relu1_c2, 5, self.config.feature_length, 1, self.config.feature_length, 20,'2_5', config = self.config)#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_7Ds"):
                    self.conv2_7_c2 = conv(self.relu1_c2, 7, self.config.feature_length, 1, self.config.feature_length, 20,'2_7', config = self.config)#conv2 :[batch_size, length, 1, channel]
                    self.bn2_7_c2 = bn(self.conv2_7_c1, self.is_training, '2_7')
                    self.relu2_7_c2 = tf.nn.relu(self.bn2_7_c2)#通过5×feat_len卷积提取特征，将所有特征按20种不同比例融合成20种新特征
                    self.relu2_7_c2 = tf.squeeze(self.relu2_7_c2, 2)   
                with tf.variable_scope("cnn_9Ds"):
                    self.conv2_9_c2 = conv(self.relu1_c2, 9, self.config.feature_length, 1, self.config.feature_length, 20,'2_9', config = self.config)#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_maxpool"):
                    self.cnn_out_c2 = tf.concat([self.relu2_3_c2, self.relu2_5_c2, self.relu2_7_c2,self.relu2_9_c2], 2)
                    self.cnn_out_c2 = tf.expand_dims(self.cnn_out_c2, 3)
                    self.cnn1_c2 = conv(self.cnn_out_c2, 3, 3, 2, 2, 128, 'cnn1', config = self.config)
                    self.cnn1_c2 = bn(self.cnn1_c2, self.is_training, '11')
                    self.maxpool1_c2 = max_pool(self.cnn1_c2)
                    self.cnn2_c2 = conv(self.maxpool1_c2, 3, 3, 2, 2, 256, 'cnn2', config = self.config)
                    self.cnn2_c2 = bn(self.cnn2_c2, self.is_training, '22')
                    self.maxpool2_c2 = max_pool(self.cnn2_c2)
                    self.cnn3_c2 = conv(self.maxpool2_c2, 3, 3, 2, 2, 512, 'cnn3', config = self.config)
                    self.cnn3_c2 = bn(self.cnn3_c2, self.is_training, '33')
                    self.maxpool3_c2 = max_pool(self.cnn3_c2, stride_h = 4,stride_w = 4)
            ...
            with tf.variable_scope("FC_part"):
                self.fc1_c2 = fc(self.BiLSTM_out3_c2, 1024, num = '1')
                self.fc1_c2 = tf.nn.relu(self.fc1_c2)
                self.fc1_c2 = tf.nn.dropout(self.fc1_c2, keep_prob = self.dropout_rate)
#                self.fc2_c2 = fc(self.fc1_c2, 512, num = '2')
        ...
        with tf.variable_scope("channel3"):
            with tf.variable_scope("CNN_part"):
                with tf.variable_scope("cnn_m20Ds"):
                    self.conv1_c3 = conv(self.inputX, 20, 1, 20, 1, 1, '1', name = 'integration', config = self.config) #conv1 :[batch_size, seq_length, feat_num, 1]
                    self.bn1_c3 = bn(self.conv1_c3, self.is_training, '1')
                    self.relu1_c3 = tf.nn.relu(self.bn1_c3)
                with tf.variable_scope("cnn_3Ds"):
                    self.conv2_3_c3 = conv(self.relu1_c3, 3, self.config.feature_length, 1, self.config.feature_length, 20,'2_3', config = self.config)#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_5Ds"):
                    self.conv2_5_c3 = conv(self.relu1_c3, 5, self.config.feature_length, 1, self.config.feature_length, 20,'2_5', config = self.config)#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_7Ds"):
                    self.conv2_7_c3 = conv(self.relu1_c3, 7, self.config.feature_length, 1, self.config.feature_length, 20,'2_7', config = self.config)#conv2 :[batch_size, length, 1, channel]
                    self.bn2_7_c3 = bn(self.conv2_7_c3, self.is_training, '2_7')
                    self.relu2_7_c3 = tf.nn.relu(self.bn2_7_c3)#通过5×feat_len卷积提取特征，将所有特征按20种不同比例融合成20种新特征
                    self.relu2_7_c3 = tf.squeeze(self.relu2_7_c3, 2) 
                with tf.variable_scope("cnn_9Ds"):
                    self.conv2_9_c3 = conv(self.relu1_c3, 9, self.config.feature_length, 1, self.config.feature_length, 20,'2_9', config = self.config)#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_maxpool"):
                    self.cnn_out_c3 = tf.concat([self.relu2_3_c3, self.relu2_5_c3, self.relu2_7_c3,self.relu2_9_c3], 2)
                    self.cnn_out_c3 = tf.expand_dims(self.cnn_out_c3, 3)
                    self.cnn1_c3 = conv(self.cnn_out_c3, 3, 3, 2, 2, 128, 'cnn1', config = self.config)
                    self.cnn1_c3 = bn(self.cnn1_c3, self.is_training, '11')
                    self.maxpool1_c3 = max_pool(self.cnn1_c3)
                    self.cnn2_c3 = conv(self.maxpool1_c3, 3, 3, 2, 2, 256, 'cnn2', config = self.config)
                    self.cnn2_c3 = bn(self.cnn2_c3, self.is_training, '22')
                    self.maxpool2_c3 = max_pool(self.cnn2_c3)
                    self.cnn3_c3 = conv(self.maxpool2_c3, 3, 3, 2, 2, 512, 'cnn3', config = self.config)
                    self.cnn3_c3 = bn(self.cnn3_c3, self.is_training, '33')
                    self.maxpool3_c3 = max_pool(self.cnn3_c3, stride_h = 4,stride_w = 4)
            ...
            with tf.variable_scope("FC_part"):
                self.fc1_c3 = fc(self.BiLSTM_out3_c3, 1024, num = '1')
                self.fc1_c3 = tf.nn.relu(self.fc1_c3)
                self.fc1_c3 = tf.nn.dropout(self.fc1_c3, keep_prob = self.dropout_rate)
#                self.fc2_c3 = fc(self.fc1_c3, 512, num = '2')
        with tf.variable_scope("final_fc"):
            self.fc_out = tf.concat([self.fc1_c1, self.fc1_c2, self.fc1_c3], 1)
            self.fc_out1 = fc(self.fc_out, 1024 , num = '123123')
            self.fc_out1 = tf.nn.tanh(self.fc_out1)
            self.inference_out = fc(self.fc_out1, 5 )
...
########################loss funcotion##################################
        ...
        self.optimizer_change = tf.train.AdamOptimizer(self.lrate, beta1 = 0.6, name = 'Adam_change')
...
###########################regular##############################3
        grads_and_vars_regular = self.optimizer_change.compute_gradients(self.loss_regular,
                                                            self.vs)
        ...
        self.apply_gradient_op_regular = self.optimizer_change.apply_gradients(
...
def conv(inputX, kheight, kwidth, stride_h, stride_w, filters_out, num,  config, name = None):
