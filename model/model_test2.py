...
########################inference funcotion##################################
    def inference(self): #inputX :[batch_size, seq_length, feat_num*20, channel]
        ...
        with tf.variable_scope("CNN_rawdata"):
            self.conv1_raw_c1 = conv(self.inputX, 3, 7, 1, 1, 1, '1')                   
            self.bn1_raw_c1 = bn(self.conv1_raw_c1, self.is_training, '1')
            self.relu1_raw_c1 = tf.nn.relu(self.bn1_raw_c1)
            self.maxpool1_raw_c1 = conv(self.relu1_raw_c1, 3,3,2,2, 1,'321')     
            self.conv2_raw_c1 = conv(self.maxpool1_raw_c1, 3, 7, 2, 1, 1, '2')
            self.bn2_raw_c1 = bn(self.conv2_raw_c1, self.is_training, '2')
            self.relu2_raw_c1 = tf.nn.relu(self.bn2_raw_c1)
            self.maxpool2_raw_c1 = conv(self.relu2_raw_c1, 3,3,2,2, 1,'3213')    
            self.conv3_raw_c1 = conv(self.maxpool2_raw_c1, 3, 7, 1, 1, 1, '3')
            self.bn3_raw_c1 = bn(self.conv3_raw_c1, self.is_training, '3')
            self.relu3_raw_c1 = tf.nn.relu(self.bn3_raw_c1)
            self.maxpool3_raw_c1 = conv(self.relu3_raw_c1, 3,3,2,2, 1,'312421')    
            self.resize3 = tf.image.resize_images(self.maxpool3_raw_c1, [75, 10], method = 1)
            self.conv4_raw_c1 = conv(self.maxpool3_raw_c1, 3, 7, 1, 1, 1, '4')
            self.bn4_raw_c1 = bn(self.conv4_raw_c1, self.is_training, '4')
            self.relu4_raw_c1 = tf.nn.relu(self.bn4_raw_c1)
            self.maxpool4_raw_c1 = max_pool(self.relu4_raw_c1, stride_h = 2, stride_w = 2)     
            self.resize4 = tf.image.resize_images(self.maxpool4_raw_c1, [75, 10], method = 1)
            self.raw_out = tf.concat([self.maxpool2_raw_c1, self.resize3, self.resize4], axis = 2)
            self.raw_out = tf.reshape(self.raw_out, [-1, 75, 30])

        with tf.variable_scope("CNN_1minute"):
            with tf.variable_scope("cnn_m20Ds"):
                ...
                self.relu1_c1 = tf.nn.relu(self.bn1_c1)
            with tf.variable_scope("cnn_3Ds"):
                self.conv2_3_c1 = conv(self.relu1_c1, 3, self.config.feature_length, 1, self.config.feature_length, 20,'2_3')#conv2 :[batch_size, length, 1, channel]
            ...
            with tf.variable_scope("cnn_5Ds"):
                self.conv2_5_c1 = conv(self.relu1_c1, 5, self.config.feature_length, 1, self.config.feature_length, 20,'2_5')#conv2 :[batch_size, length, 1, channel]
            ...
            with tf.variable_scope("cnn_9Ds"):
                self.conv2_9_c1 = conv(self.relu1_c1, 9, self.config.feature_length, 1, self.config.feature_length, 20,'2_9')#conv2 :[batch_size, length, 1, channel]
            ...
            with tf.variable_scope("cnn_maxpool"):
                ...
                self.cnn1_c1 = conv(self.cnn_out_c1, 3, 3, 1, 1, 4, 'cnn1')
                self.cnn1_bn_c1 = bn(self.cnn1_c1, self.is_training, '11')
                self.cnn1_relu_c1 = tf.nn.relu(self.cnn1_bn_c1)
                self.maxpool1_c1 = max_pool(self.cnn1_relu_c1, 3, 3, 1, 2)
                self.cnn2_c1 = conv(self.maxpool1_c1, 3, 3, 1, 1, 8, 'cnn2')
                self.cnn2_bn_c1 = bn(self.cnn2_c1, self.is_training, '22')
                self.cnn2_relu_c1= tf.nn.relu(self.cnn2_bn_c1)
                self.maxpool2_c1 = max_pool(self.cnn2_relu_c1, 3, 3, 1, 2)
                self.cnn3_c1 = conv(self.maxpool2_c1, 3, 3, 1, 2, 16, 'cnn3')
                self.cnn3_bn_c1 = bn(self.cnn3_c1, self.is_training, '33')
                self.cnn3_relu_c1 = tf.nn.relu(self.cnn3_bn_c1)
                self.maxpool3_c1 = max_pool(self.cnn3_relu_c1, 3, 3, 1, 2)
                self.maxpool3_c1 = tf.reshape(self.maxpool3_c1, [-1, 30, 64, 1])
                self.dense_out = tf.image.resize_images(self.maxpool4_raw_c1, [75, 64], method = 1)
                self.dense_out = tf.reshape(self.dense_out, [-1 , 75, 64])

        with tf.variable_scope("BiLSTM_part"):              #BiLSTM部分，3个隐藏层
            with tf.variable_scope("BiLSTM_1"):
                self.BiLSTM_in_c1 = tf.concat([self.raw_out, self.dense_out], axis = 2)
                self.BiLSTM_out1_c1 = BiLSTM(self.BiLSTM_in_c1, 200)
            with tf.variable_scope("BiLSTM_2"):
                self.BiLSTM_out2_c1 = BiLSTM(self.BiLSTM_out1_c1, 200)
            with tf.variable_scope("BiLSTM_3"):
                self.BiLSTM_out3_c1 = BiLSTM(self.BiLSTM_out2_c1, 200)
            self.BiLSTM_out3_c1 = tf.reshape(self.BiLSTM_out3_c1, [-1, 75*400])

        with tf.variable_scope("FC_part"):                  #全连接层将400维特征缩小为200维，再缩小为1。即：预测值
            self.fc1 = fc(self.BiLSTM_out3_c1, 2048, num = '1')
            self.fc1 = tf.nn.tanh(self.fc1)
            self.fc2 = fc(self.fc1, 1024, num = '2')
            self.fc2 = tf.nn.tanh(self.fc2)
            self.inference_out = fc(self.fc2, 5 )
        ...
        self.ema = tf.train.ExponentialMovingAverage(self.config.MOVING_AVERAGE_DECAY, self.global_step)
        self.update_losses = self.ema.apply([self.loss])
        tf.add_to_collection(UPDATE_OPS_COLLECTION, self.update_losses)
        ...
        self.optimizer_change = tf.train.AdamOptimizer(self.lrate, name = 'Adam_change')
...
###########################regular##############################3
        grads_and_vars_regular = self.optimizer.compute_gradients(self.loss_regular,
                                                            self.vs)
        ...
        self.apply_gradient_op_regular = self.optimizer.apply_gradients(
...
'''
        ...
        with tf.variable_scope("channel2"):
            with tf.variable_scope("CNN_rawdata"):
                self.conv1_raw_c2 = conv(self.inputX, 3, 7, 1, 1, 6, '1')
                self.bn1_raw_c2 = bn(self.conv1_raw_c2, self.is_training, '1')
                self.tanh1_raw_c2 = tf.nn.tanh(self.bn1_raw_c2)
                self.maxpool1_raw_c2 = max_pool(self.tanh1_raw_c2, stride_h = 2, stride_w = 2)

                self.conv2_raw_c2 = conv(self.maxpool1_raw_c2, 3, 7, 1, 1, 8, '2')
                self.bn2_raw_c2 = bn(self.conv2_raw_c2, self.is_training, '2')
                self.tanh2_raw_c2 = tf.nn.tanh(self.bn2_raw_c2)
                self.maxpool2_raw_c2 = max_pool(self.tanh2_raw_c2, stride_h = 2, stride_w = 2)

                self.conv3_raw_c2 = conv(self.maxpool2_raw_c2, 3, 7, 1, 1, 8, '3')
                self.bn3_raw_c2 = bn(self.conv3_raw_c2, self.is_training, '3')
                self.tanh3_raw_c2 = tf.nn.tanh(self.bn3_raw_c2)
                self.maxpool3_raw_c2 = max_pool(self.tanh3_raw_c2, stride_h = 2, stride_w = 2)

                self.recomb1_c2 = fc(tf.reshape(self.maxpool1_raw_c2, [-1, 300*20*6]), 3600, 'recomb1')
                self.recomb1_c2 = tf.nn.relu(self.recomb1_c2)
                self.recomb2_c2 = fc(tf.reshape(self.maxpool2_raw_c2, [-1, 150*10*8]), 1200, 'recomb2')
                self.recomb2_c2 = tf.nn.relu(self.recomb2_c2)
                self.recomb3_c2 = fc(tf.reshape(self.maxpool3_raw_c2, [-1, 75*5*8]), 300, 'recomb3')
                self.recomb3_c2 = tf.nn.relu(self.recomb3_c2)
                self.recomb_c2 = tf.concat([self.recomb1_c2, self.recomb2_c2, self.recomb3_c2], axis = 1)
                self.recomb_c2 = tf.expand_dims(self.recomb_c2, 1)

            with tf.variable_scope("CNN_part"):
                with tf.variable_scope("cnn_m20Ds"):
                    self.conv1_c2 = conv(self.inputX, 20, 1, 20, 1, 1, '1', name = 'integration') #conv1 :[batch_size, seq_length, feat_num, 1]
                    self.bn1_c2 = bn(self.conv1_c2, self.is_training, '1')
                    self.relu1_c2 = tf.nn.relu(self.bn1_c2)
                with tf.variable_scope("cnn_3Ds"):
                    self.conv2_3_c2 = conv(self.relu1_c2, 3, self.config.feature_length, 1, self.config.feature_length, 20,'2_3')#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_5Ds"):
                    self.conv2_5_c2 = conv(self.relu1_c2, 5, self.config.feature_length, 1, self.config.feature_length, 20,'2_5')#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_7Ds"):
                    self.conv2_7_c2 = conv(self.relu1_c2, 7, self.config.feature_length, 1, self.config.feature_length, 20,'2_7')#conv2 :[batch_size, length, 1, channel]
                    self.bn2_7_c2 = bn(self.conv2_7_c2, self.is_training, '2_7')
                    self.relu2_7_c2 = tf.nn.relu(self.bn2_7_c2)#通过7×feat_len卷积提取特征，将所有特征按20种不同比例融合成20种新特征
                    self.relu2_7_c2 = tf.squeeze(self.relu2_7_c2, 2)
                with tf.variable_scope("cnn_9Ds"):
                    self.conv2_9_c2 = conv(self.relu1_c2, 9, self.config.feature_length, 1, self.config.feature_length, 20,'2_9')#conv2 :[batch_size, length, 1, channel]
                ...
                with tf.variable_scope("cnn_maxpool"):
                    self.cnn_out_c2 = tf.concat([self.relu2_3_c2, self.relu2_5_c2, self.relu2_7_c2, self.relu2_9_c2], 2)#将新特征组合在一起
                    self.cnn_out_c2 = tf.expand_dims(self.cnn_out_c2, 3)
                    self.cnn1_c2 = conv(self.cnn_out_c2, 3, 3, 2, 2, 16, 'cnn1')
                    self.cnn1_bn_c2 = bn(self.cnn1_c2, self.is_training, '11')
                    self.cnn1_relu_c2 = tf.nn.relu(self.cnn1_bn_c2)
                    self.maxpool1_c2 = max_pool(self.cnn1_relu_c2)

                    self.cnn2_c2 = conv(self.maxpool1_c2, 3, 3, 1, 1, 64, 'cnn2')
                    self.cnn2_bn_c2 = bn(self.cnn2_c2, self.is_training, '22')
                    self.cnn2_relu_c2= tf.nn.relu(self.cnn2_bn_c2)
                    self.maxpool2_c2 = max_pool(self.cnn2_relu_c2)

                    self.cnn3_c2 = conv(self.maxpool2_c2, 3, 3, 2, 2, 512, 'cnn3')
                    self.cnn3_bn_c2 = bn(self.cnn3_c2, self.is_training, '33')
                    self.cnn3_relu_c2 = tf.nn.relu(self.cnn3_bn_c2)
                    self.maxpool3_c2 = max_pool(self.cnn3_relu_c2)
                    self.maxpool3_c2 = tf.reshape(self.maxpool3_c2, [-1, 1, 3*512])

            with tf.variable_scope("BiLSTM_part"):
                with tf.variable_scope("BiLSTM_1"):
                    self.BiLSTM_in_c2 = tf.concat([self.recomb_c2, self.maxpool3_c2], axis = 2)
                    self.BiLSTM_out1_c2 = BiLSTM(self.BiLSTM_in_c2, self.config.lstm_units)
                with tf.variable_scope("BiLSTM_2"):
            ...
            with tf.variable_scope("FC_part"):
                self.fc1_c2 = fc(self.BiLSTM_out3_c2, 2 * self.config.lstm_units, num = '1')
                self.fc1_c2 = tf.nn.tanh(self.fc1_c2)
                self.fc1_c2 = tf.nn.dropout(self.fc1_c2, keep_prob = self.dropout_rate)
                self.fc2_c2 = fc(self.fc1_c2, self.config.lstm_units, num = '2')
'''